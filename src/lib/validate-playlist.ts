...
export function validatePlaylistResponse(data: unknown): PlaylistResponse {
  ...
  if (
    ...
    throw new Error(
      `Invalid inference.energy: must be integer 1-5, got ${inference.energy}`,
    );
  }
  if (
    typeof inference.vocal_style !== "string" ||
    !VALID_VOCAL_STYLES.includes(
      inference.vocal_style as (typeof VALID_VOCAL_STYLES)[number],
    )
  ) {
  ...
  const tracks: PlaylistTrack[] = obj.tracks.map(
    (track: unknown, i: number) => {
      ...
      if (
        ...
        !VALID_TEMPOS.includes(t.tempo as (typeof VALID_TEMPOS)[number])
      ) {
  ...
  return {
    inference: {
      ...
      constraints: {
        avoid: Array.isArray(
          (inference.constraints as Record<string, unknown>)?.avoid,
        )
          ? ((inference.constraints as Record<string, unknown>)
              .avoid as string[])
          : [],
        prefer: Array.isArray(
          (inference.constraints as Record<string, unknown>)?.prefer,
        )
          ? ((inference.constraints as Record<string, unknown>)
              .prefer as string[])
          : [],
